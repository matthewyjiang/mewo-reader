...
**Account drawer.** Left sheet, square corners, 300dp. Dummy profile at the top: letter avatar, name You, handle @local on the phone shelf or @username once you sign into a hosted server. Hairline, then one Settings row. Tap it to close the drawer and open Settings. The drawer does not hold display or library controls. A horizontal swipe from the left edge opens it on Home, Search, and Likes. The system Back button on Home sends the app to the background. The reader and Settings keep Back for leaving those screens; that edge does not open the drawer.

**Settings.** Full-screen destination. Back and a centered title. The home page is two rows, Display and Library, each with a mute line and a chevron. Hairline between them. No controls on this page. Tap a row to open that nested page. Back from a nested page returns to Settings. Back from Settings returns to the feed you came from. The tab bar hides while Settings is open.

**Display.** Nested Settings page. Four swatches in a 2x2: Twitter light, X light, Twitter dim, Lights out. Selecting a swatch applies it and stays on the page. Below them, **Mewo mode** is an accessible switch that uses the supplied Teddy artwork for the launcher icon, launch screen, and home header mark without changing the palette. Off by default; turning it off restores the open-book launcher/launch logo and the theme's bird or X header. The persisted Android launcher-component selection is the mode's source of truth, avoiding a separate preference that could disagree after backup restore. Launcher aliases point to short-lived themed entry activities, which hand off to the always-enabled reader activity. This keeps icon changes from dismissing Settings and preserves the open book when returning from the launcher.

**Library.** Nested Settings page. Two choices: On this phone, or Hosted server. Local keeps EPUB files, likes, and reading place on the device. Hosted is a second shelf. Pick it and the page shows a server URL, username, password, Sign in, and Create account. After sign-in the drawer handle is that username and Sign out is the way out. The shelves do not merge.
