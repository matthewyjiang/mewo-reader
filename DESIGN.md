...
**Account drawer.** Left sheet, square corners, 300dp. Dummy profile at the top: letter avatar, name You, handle @local on the phone shelf or @username once you sign into a hosted server. Hairline, then Settings. Display lives here: four swatches in a 2x2, Twitter light, X light, Twitter dim, Lights out. Selecting a swatch applies it and keeps the drawer open. Below them, **Mewo mode** is an accessible switch that uses the supplied Teddy artwork for the launcher icon, launch screen, and home header mark without changing the palette. Off by default; turning it off restores the open-book launcher/launch logo and the theme's bird or X header. Below that, **Library** is two choices: On this phone, or Hosted server. Local keeps EPUB files, likes, and reading place on the device. Hosted is a second shelf. Pick it and the drawer shows a server URL, username, password, Sign in, and Create account. After sign-in the handle is that username and Sign out is the way out. The shelves do not merge. The drawer scrolls on smaller screens. A horizontal swipe from the left edge opens it on Home, Search, and Likes. The system Back button on Home sends the app to the background. The reader keeps Back for leaving the book; that edge does not open the drawer. The persisted Android launcher-component selection is the mode's source of truth, avoiding a separate preference that could disagree after backup restore. Launcher aliases point to short-lived themed entry activities, which hand off to the always-enabled reader activity. This keeps icon changes from dismissing the drawer and preserves the open book when returning from the launcher.
